...
export class MonthGridView extends ItemView {
  ...
  async render(): Promise<void> {
    ...
    toolbar.createEl("strong", { text: monthLabel(this.year, this.month) });
    ...
    data.flatMap((entry) => entry.items).forEach((item) => {
    ...
    for (let index = 0; index < total; index += 1) {
      ...
      if (day < 1 || day > days) { cell.addClass("is-outside"); continue; }
